...
void map_segment(Um_instruction inst, UM um)
{
    ...
    *num_words_p = num_words;
    ...
    if (Seq_length(um->unmappedID) != 0) {
        ...
        um->registers[get_reg_i(inst, 'b')] = vector_length(um->segmented_memory)
                                              - 1;
    }
}
...
void unmap_segment(Um_instruction inst, UM um)
{
    ...
    /* Unmaps the segment */
    free(((Um_instruction *)vector_get(um->segmented_memory, unmappedID)));
...
void load_program(Um_instruction inst, UM um) 
{
    ...
    if (bv != 0) {
        ...
        for (unsigned i = 0; i < length_of_seq; i++) {
            new_segment[i] = 
                      ((Um_instruction *)vector_get(um->segmented_memory, bv))[i];
        }
        ...
        Seq_put(um->segment_lengths, 0, length_p);
    }
...
uint32_t get_reg_i(Um_instruction instruction, char character) {
    ...
    if (character == 'c') {
        registerIndex = bp_get_u(instruction, 3, 0);
    } else if (character == 'b') {
        registerIndex = bp_get_u(instruction, 3, 3);
    } else if (character == 'a') {
        registerIndex = bp_get_u(instruction, 3, 6);
    } 
