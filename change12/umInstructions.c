...
void execute_instr(UM um)
{
    ...
    while (op_code != 7) {
        ...
        ((Um_instruction *)um->segmented_memory->array[0])[um->program_counter]; 
        ...
        switch(op_code) {
            ...
            case 1: //segment load
                ...
                // if (bv >= (uint32_t)um->segmented_memory->size) {
                //     fprintf(stderr, "Out of index!");
                //     abort();
                // }
                // Um_instruction inst_at_rbrc = um->segmented_memory->array[bv][cv];
